...
function fetchBusiness(){
    ...
    function storeDetails(businessId){
        ...
        const detailsUrl = `http://localhost:8001/api-gateway/public/inspection/fetch/${businessId}`;
        fetch(detailsUrl)
            .then(response => response.json())
            .then(data => {
    ...
    function search(searchTerm){
        ...
        tableBody.innerHTML = ''; // This will remove all the previous rows
        // Fetch data from the API endpoint to populate the table
    fetch(`http://localhost:8001/api-gateway/public/inspection/schedule/${searchTerm}`)
        .then(response => response.json())
        .then(data => {
            const tableBody = document.querySelector("#marketsTable tbody");
            tableBody.innerHTML = '';
    ...
    //let bookings = [];
    const detailsUrl = 'http://localhost:8001/api-gateway/public/inspection/inspectionSchedule';
    fetch(detailsUrl)
        .then(response => response.json())
        .then(data => {
            if (data) {
    ...
    const scheduleForm = document.getElementById('schedule-form');
        ...
        scheduleForm.addEventListener('submit', (event) => {
            ...
            if (isValid){
                ...
                const detailsUrl = 'http://localhost:8001/api-gateway/public/inspection/schedule';
                fetch(detailsUrl, {
                    method: 'POST',
                    headers: {
...
function scheduleValidation(){
    ...
    let assignedMonths = [];
    const fetchUrl = `http://localhost:8001/api-gateway/public/inspection/inspectors/${inspectionDate}`;
    fetch(fetchUrl, {
        method: 'GET'
    })
    .then(response => response.json())
...
const fetchUrl = `http://localhost:8001/api-gateway/public/inspection/scheduleValidation/${timeFrom}/${timeTo}/${inspectionDate}`;
fetch(fetchUrl, {
    method: 'GET',
})
.then(response => response.json())
